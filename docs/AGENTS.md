...
当坤哥让我执行以下任务时，全部进入反黑箱透明化执行：

- **调研类**：市场调研、竞品分析、数据抓取、用户研究等
- **修复类**：Bug修复、问题排查、错误修复、调试等 ← 新增
- **审计类**：代码审计、安全审计、合规检查等
- **开发类**：编写脚本、开发功能、代码实现等
- **分析类**：数据分析、报告生成、文档整理等
...
**📨 进度推送方式：必须走飞书 Webhook，不走主会话回复**

| 方式 | 适用场景 |
|------|---------|
| **飞书 Webhook 卡片** | 反黑箱进度、每步完成通知（不卡主会话） |
| **主会话文字回复** | 最终报告、汇总结果、坤哥提问 |

**为什么不能走主会话**：主会话回复会让坤哥的消息列表被切碎，干扰正常对话。反黑箱进度推送必须独立走飞书群 Webhook。

**执行规则：**
- 调研/抓取类任务：每抓完1个数据源，立即发 Webhook 卡片推送中间结果
- 代码任务：每完成1个文件/模块，立即发卡片通知
- 卡片格式：简洁、标题醒目、包含关键数据（不是完整报告）
- 全部完成后，在主会话发最终汇总报告（带完整分析）

**违反后果**：主会话被进度消息淹没，坤哥无法正常对话。

**🚨 执行强制检查（每次任务必须通过）**

每次接到坤哥的任务时，我必须在执行前做以下检查：

```
任务开始前检查清单：
□ 1. 明确总步骤数和每步内容
□ 2. 立即调用 feishu_progress.py 推送"任务开始"卡（颜色=blue）
□ 3. 每步完成后立即调用 feishu_progress.py 推送进度卡（颜色=blue）
□ 4. 全部完成后调用 feishu_progress.py 推送"完成"卡（颜色=green/done）
□ 5. 所有 Webhook 卡发送完毕，才能在主会话发最终报告
```

**如果我在任务进行中在主会话报告了任何中间进度，说明我没有通过检查。**

**强制机制**：任何超过 2 步的任务，第一步必须是推送 Webhook 启动卡。在那之前不做任何主会话回复。

**技术实现：直接调用推送脚本，不在主会话说话**
...
**脚本位置**：`/root/.openclaw/workspace/scripts/feishu_progress.py`

**Webhook 配置**：已使用坤哥提供的 `375a8be1-...`

**使用规则**：
- 调研任务每步完成后 → 调用脚本推送 Webhook 卡
- 主会话只回复最终汇总报告
- 脚本已配置正确的 Webhook，无需手动填
